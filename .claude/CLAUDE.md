...
- When adding a species class, follow the playbook at [`docs/new-species-onboarding.md`](../docs/new-species-onboarding.md). Lockstep edits in `CatchPhotoAnalyzer.swift`: `speciesLabels` (matches Python `ImageFolder` alphabetical order — not appended), `regressorBypassSpecies` (single `static let`), `speciesDisplayToLabel` (every value must exist in `speciesLabels` — drift here silently lands the regressor on `speciesIdx=0`), and `speciesLengthRanges` (always provide a heuristic biological range — without an entry the species inherits a steelhead-shaped 10–47" envelope); `speciesDisplayNames` in `CatchChatViewModel.swift`; plus a retrained `ViTFishSpecies.mlpackage` **and** `LengthRegressor.mlmodel` — the regressor consumes `species_index` as a feature, so alphabetical insertion silently breaks it without retraining.
