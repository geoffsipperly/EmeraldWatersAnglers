...
- When adding a species class, follow the playbook at [`docs/new-species-onboarding.md`](../docs/new-species-onboarding.md). Lockstep edits: `speciesLabels` (matches Python `ImageFolder` alphabetical order — not appended) and `regressorBypassSpecies` (two call sites, use `replace_all`) in `CatchPhotoAnalyzer.swift`; `speciesDisplayNames` in `CatchChatViewModel.swift`; plus a retrained `ViTFishSpecies.mlpackage` **and** `LengthRegressor.mlmodel` — the regressor consumes `species_index` as a feature, so alphabetical insertion silently breaks it without retraining.
